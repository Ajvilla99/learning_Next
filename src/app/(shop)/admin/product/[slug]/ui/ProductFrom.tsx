...
import { createUpdateProduct, deleteProducImage } from "@/actions";
import { ProductImage } from "@/components";
import { Category, Product, ProductImage as ProducWithImage } from "@/interfaces";
import clsx from "clsx";
import { useRouter } from "next/navigation";
import { useForm } from "react-hook-form";

interface Props {
  product: Partial<Product> & { ProductImage?: ProducWithImage[] };
  categories: Category[];
}
...
export const ProductForm = ({ product, categories }: Props) => {
  ...
  const onSizeChanged = ( size: string ) => {
    ...
    const sizes = new Set(getValues('sizes')); // Set No acepta duplicados
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {
              product.ProductImage?.map( image => (
                ...
                <div key={ image.id } >
                  ...
                  <button
                    type="button"
                    onClick={ () => deleteProducImage( image.id, image.url )}
